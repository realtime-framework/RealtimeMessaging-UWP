    ...
    internal class MsgProcessor {
        ...
        internal void ProcessOperationValidated(string arguments) {
            if (!String.IsNullOrEmpty(arguments)) {
                client._reconnectStartedAt = null;
                ...
                if (isValid) {
                    ...
                    if (client._alreadyConnectedFirstTime) {
                        ...
                        for (int i = 0; i < channelsToRemove.Count; i++) {
                            ...
                            client._subscribedChannels.TryRemove(channelsToRemove[i].ToString(), out removeResult);
                        }
                    ...
                    if (arguments.IndexOf("busy") < 0) {
                        if (client._reconnectTimer.IsActive() /* != null*/) {
                            //_reconnectTimer.Stop();
                            client._reconnectTimer.Dispose();
                        }
                    }
                    ...
                    lock (client._multiPartMessagesBuffer) {
                        ...
                        if (!String.IsNullOrEmpty(messageId)) {
                            if (!client._multiPartMessagesBuffer.ContainsKey(messageId)) {
                                client._multiPartMessagesBuffer.TryAdd(messageId, new ConcurrentDictionary<int, BufferedMessage>());
                            }
